...
def main():
    ...
    is_interactive = len(non_flag_args) == 0
    ...
    while True:
        ...
        print("\n" + "="*50)
        print("=== Auto Run Scraping Platform ===")
        print("="*50)
        print("\n=== Daftar Kegiatan ===")
        ...
        if non_flag_args:
            ...
            print("\nMasukkan nomor profil yang ingin dijalankan (misal 1,3,4 atau 'all').")
            pilihan = input("Pilihan Anda (Ketik 0 untuk Keluar): ").strip().lower()
        
        if pilihan == "0":
            print("\n[INFO] Keluar dari program. Sampai jumpa!")
            break
        ...
        if not selected_indices:
            print("[ERROR] Tidak ada profil valid yang dipilih.")
            if not is_interactive:
                break
            continue
            
        print(f"\n[INFO] Memproses profil nomor: {', '.join(map(str, selected_indices))}")
        ...
        print("\n[SUCCESS] Semua kegiatan yang dipilih telah selesai diproses!")
